...
app.listen(port, () => {
  ...
  function validateDiagonal(node){
    let validNode = true;
    ...
    if (validNode) {
        gameState.diagonalArray.push(tempArray);
        return true;
    } else {
        return false;
    }
  ...
  function moveValidation(node){
    ...
    if (matchCoords(node,gameState.openNodeArray) === -1){
        ...
        if (xDiff === 0 || yDiff === 0 || isDiagonal) {
            ...
            if (isDiagonal) {
                validDirection = validateDiagonal(node);
            };
        } else {
  ...
  function lineEnd(node, player){
    ...
    if (validMove) {
        ...
        //Restore initial node
        gameState.openNodeArray.push([gameState.storedNode.x, gameState.storedNode.y]); 
